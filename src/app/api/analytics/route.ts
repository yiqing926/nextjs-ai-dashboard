import { DailyMetrics } from '../../[locale]/type/analytics';
...
export async function GET() {
  try {
    ...
    // 动态从远端的 Supabase 捞取最近 7 天的真实数据
    const dbData = await db.dailyMetric.findMany({
      orderBy: { date: 'asc' },
      take: 7, 
    });

    const realData:DailyMetrics[] = dbData.map((item) => ({
      date: item.date,
    ...
    if (realData.length === 0) {
      return NextResponse.json({
        summary: { totalRevenue: 0, avgActiveUsers: 0, avgConversionRate: 0 },
        chartData: [],
      });
    }
    ...
    const totalRevenue = realData.reduce((sum, item) => sum + item.revenue, 0);
    const avgActiveUsers = Math.round(
      realData.reduce((sum, item) => sum + item.activeUsers, 0) / realData.length
    );
    const avgConversionRate = parseFloat(
      (realData.reduce((sum, item) => sum + item.conversionRate, 0) / realData.length).toFixed(2)
    );
